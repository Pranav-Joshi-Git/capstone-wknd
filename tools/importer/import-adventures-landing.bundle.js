...
var CustomImportScript = (() => {
  ...
  function parse2(element, { document }) {
    ...
    items.forEach((item) => {
      ...
      const href = titleLink && titleLink.getAttribute("href") || item.querySelector("a[href]") && item.querySelector("a[href]").getAttribute("href");
      const content = [];
      if (titleText) {
  ...
  // tools/importer/import-adventures-landing.js
  var parsers = {
    "tabs-adventure": parse,
    "cards-adventure": parse2
  };
  var transformers = [
    transform
  ];
